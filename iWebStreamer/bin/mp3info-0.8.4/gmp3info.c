...
void read_new_mp3(GtkWidget *button, GtkWidget *file) {
	...
	filename = gtk_file_selection_get_filename(GTK_FILE_SELECTION(file));
...
void quick_popup(char *title, char *message) {
	...
	popupwin_main = GTK_DIALOG(popupwin)->vbox;
        gtk_container_add(GTK_CONTAINER(popupwin),popupwin_main);
	popupwin_msg=gtk_label_new(message);
