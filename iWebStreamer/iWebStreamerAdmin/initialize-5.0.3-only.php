<?
	/*
		...
		This is the same as the main initialize.php file except the default time zone isn't initialized.
	*/
	clearstatcache ();
	...
	//clean up the upload directory
		...
		function makeSimpleFileName($fileName){
			$j=strlen($fileName)-1;
			$simple_fileName='';
			for($i=$j;$i>-1;$i--){
				...
				if($fileName[$i]=='/'||$fileName[$i]=='\\'){
					break;
				}elseif(!preg_match('/(\w|\.)/', $fileName[$i])){
				$simple_fileName='_'.$simple_fileName;
				...
				//necessaryily mean it was an end of a directory.
					...
					$simple_fileName=$fileName[$i].$simple_fileName;
				}
