...
<?
	...
	if(!isset($_GET['podcastId'])){
		$iWebStreamerErrorObj->registerCritical("Invalid url.  A podcast id is need to access 
		this page.");
	}
	...
	if(!$podcastObj){
		$iWebStreamerErrorObj->registerCritical("Invalid podcast id.  Podcast does not exist.");
	}
...
<form name="addPodcast" action="pod_managerEditPodcast_process.php" method="POST" enctype="multipart/form-data" onSubmit="return validateForm()" id="addPodcast">
<input type="hidden" name="podcastId" value="<?echo $podcastObj->getPodcastId();?>">
<table align="center" cellpadding="0" cellspacing="4" border="0">
	<tr align="center" valign="center">
		...
		<td align="left">
			 <input type="text" name="title" id="title" value="<?echo $podcastObj->getTitle();?>">
		</td>
		<td align="left">
	...
	<tr valign="center" bgcolor="#F1F2F8">
		...
		<td align="left" nowrap>
			...
			http://yourwebsite.com/related_link.html</i><br>
			 <input type="text" name="link" id="link" value="<?echo $podcastObj->getLink();?>">
		</td>
		<td align="left">
	...
	<tr align="center" valign="center">
		...
		<td align="left">
			 <input type="text" name="copyright" id="copyright" value="<?echo $podcastObj->getCopyright();?>">
		</td>
		<td align="left">
	...
	<tr align="center" valign="center" bgcolor="#F1F2F8">
		...
		<td colspan="2" align="left">
			 <input type="text" name="author" id="author" value="<?echo $podcastObj->getAuthor();?>">
		</td>
	</tr>
	
	<?
		$block=$podcastObj->getBlock();
	?>
	<tr valign="center">
		<td align="right">
	...
	<tr valign="center">
		...
		<?
			$catArr=$podcastInfoObj->getItunesCategoriesArr();
				...
				$currCatArr=$podcastObj->getCategory();
				$MainCategory1=$currCatArr['MainCategory1'];
				$MainCategory2=$currCatArr['MainCategory2'];
				$MainCategory3=$currCatArr['MainCategory3'];
				$SubCategory1=$currCatArr['SubCategory1'];
				$SubCategory2=$currCatArr['SubCategory2'];
				$SubCategory3=$currCatArr['SubCategory3'];
					...
					<td align="left" nowrap>
						...
						<?
							foreach($catArr as $mainCat => $subCatArr){
								$selected='';
								if(strcmp($mainCat, $MainCategory1)==0){
									$selected=' selected';
								}
								echo "<option value='$mainCat'$selected>$mainCat</option>\n";
							}
						?>
						</select>
					</td>
				...
				<tr>
					...
					<td align="left" nowrap>
						...
						<?
							foreach($catArr as $mainCat => $subCatArr){
								$selected='';
								if(strcmp($mainCat, $MainCategory2)==0){
									$selected=' selected';
								}
								echo "<option value='$mainCat'$selected>$mainCat</option>\n";
							}
				...
				<tr bgcolor="#F1F2F8">
					...
					<td align="left" nowrap>
						...
						<?
							foreach($catArr as $mainCat => $subCatArr){
								$selected='';
								if(strcmp($mainCat, $MainCategory3)==0){
									$selected=' selected';
								}
								echo "<option value='$mainCat'$selected>$mainCat</option>\n";
							}
	...
	<tr valign="center" bgcolor="#F1F2F8">
		...
		<td align="left" nowrap>
			 <select name="explicit">
				...
				<? 
					if($podcastObj->getExplicit()=='clean'){
						echo "selected";
					}
				?>
		...
		<td align="left">
			...
			you can leave this to clean or no, and then set the explict tag to yes only on specific episodes.
			<ul>
	...
	<script language="javascript" type="text/javascript">
		...
		<?
			foreach(preg_split('/\,/', $podcastObj->getKeywords()) as $index => $value){
				$value=trim($value);
				if(strlen($value)>0){
	...
	<tr valign="center" bgcolor="#F1F2F8">
		...
		<td align="left" nowrap>
			<input type="text" name="name" id="name" value="<?echo $podcastObj->getName();?>">
		</td>
		<td align="left">
	...
	<tr valign="center">
		...
		<td align="left" nowrap>
			<input type="text" name="email" id="email" value="<?echo $podcastObj->getEmail();?>">
		</td>
		<td align="left">
	...
	<tr valign="center" bgcolor="#F1F2F8">
		...
		<td align="left" nowrap>
			<textarea name="subtitle" cols="30" rows="3"><?echo $podcastObj->getSubtitle();?></textarea>
		</td>
		<td align="left">
			No official limitations are placed on the subtitle, but iTunes recommends limiting usage to a few words.
		</td>
	</tr>
	...
	<tr valign="center">
		...
		<td align="left" nowrap>
			...
			onKeyUp="textCounter(document.addPodcast.summary,document.addPodcast.remLen1,4000)"><?echo $podcastObj->getSummary();?></textarea>
		...
		<td align="left">
			...
			<br>
			<br>
			<input readonly type="text" name="remLen1" size="3" maxlength="3" value="<?echo 4000-strlen($podcastObj->getSummary());?>">
			characters left
			<br>
		</td>
	</tr>
	...
	<tr align="center">
		...
		<td bgcolor="#F1F2F8">
			<input class="button" type="button" id="submitButton" value="Edit Podcast" onClick="validateForm()">
		</td>
...
<script>
	...
	function setOptionValue(the_select, the_value)
	{
		the_select=document.getElementById(the_select);
  		for (i=0; i < the_select.options.length; i++)
  		{
    			if(the_select.options[i].text == the_value){
	...
	<?
		echo "setOptionValue('Sub Category 1', '$SubCategory1');\n";
		echo "setOptionValue('Sub Category 2', '$SubCategory2');\n";
		echo "setOptionValue('Sub Category 3', '$SubCategory3');\n";
	?>
