...
<table cellspacing="0" cellpadding="1" class="toolBar" align="center" border="1" bordercolor="#C7C7FF">
		<form action="stream_managerAddTrackFromRFAudio_process.php?webStreamId=<?echo $webStreamId;?>" method="post" enctype="multipart/form-data" 
				name="form1" 
				target="_self" 
					...
					foreach(array_reverse($rfaudio_infoObj->getRfAudioFileArr(), true) as $rfAudioFile_id => $RfAudioFileObj){
						...
						foreach($webStreamObj->getWebStreamFileArr() as $webStreamFileId => $webStreamFileObj){
							...
							if($webStreamFileObj->getIsRFAudio()){
								if($webStreamFileObj->getFileId()==$rfAudioFile_id){
									$alreadyAdded=true;
								}
							}
